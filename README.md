...
- [License & Disclaimer](#license--disclaimer)2. **Junk Masking:**
   ...
   - After every 2 real characters in the header and encrypted output, 1 random alphanumeric junk character is inserted.

---   - During decryption, these junk characters are reliably filtered out.
...
- **Detection Challenges:**Generate random key? (y/n): y

  - Junk-masked output breaks simple regex, YARA, and entropy-based rules.Enter text to encrypt: Attack at dawn!

  - Polymorphic output means no two encryptions are alike, complicating hash- or signature-based detection.[+] Output written to enc.log
...
## Comparison to Standard Encryption---

| Feature                | SynCrypt                | AES (CBC/CTR)         | ChaCha20/RC4           | XOR/Stream Ciphers     |

|------------------------|-------------------------|-----------------------|------------------------|------------------------|## FAQ

| Key Size               | 256 bits (32 bytes)     | 128/192/256 bits      | 128/256 bits           | Any                    |

| Header Size            | 48 bytes (masked)       | 16 bytes (IV)         | 8-12 bytes (nonce)     | None/Optional          |**Q: Is SynCrypt cryptographically secure?**

| Output Polymorphism    | High (junk, seed+nonce, format) | Low (IV/nonce only)   | Low (nonce only)       | None                   |- No. SynCrypt is designed for obfuscation, polymorphism, and red/blue team research, not for high-assurance cryptographic protection.

| Output Obfuscation     | Junk-masked, multi-format | None (raw/base64)     | None (raw/base64)      | None                   |

| Detection Resistance   | High (junk, variable)   | Low                   | Low                    | Very Low               |**Q: Can SynCrypt output be detected?**

| Standardized           | No (custom)             | Yes                   | Yes                    | No                     |- While junk masking and polymorphism defeat naive detection, advanced statistical or behavioral analysis can still reveal usage.

| Performance            | Moderate                | High                  | High                   | Very High              |

| Cryptanalysis          | Not peer-reviewed       | Peer-reviewed         | Peer-reviewed          | Weak                   |**Q: Can I use SynCrypt for file encryption?**
...
## Author
- Renamed and maintained as **SynCrypt**
- Original code: supercrypt_tool.c
